...
export default function HomePage() {
  ...
  // --- CAROUSEL LOADERS ---
  useEffect(() => {
    // Trending Now: Just use "trending" as an arbitrary query for hybrid
    fetch(`/api/recommend`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ query: "trending", top_n: 12 }),
    })
      .then(res => res.json())
    ...
    fetch(`/api/recommendations`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ type: "genre", query: "Mystery", limit: 12 }),
    })
      .then(res => res.json())
    ...
    fetch(`/api/recommendations`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ type: "genre", query: "Science Fiction", limit: 12 }),
    })
      .then(res => res.json())
    ...
    fetch(`/api/recommendations`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ type: "genre", query: "Romance", limit: 12 }),
    })
      .then(res => res.json())
      .then(data => setRomanceBooks(data.recommendations ?? []))
      .catch(() => setRomanceBooks([]))

    // Recently Added (use filter by year > e.g. 2018)
    fetch(`/api/recommendations`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
